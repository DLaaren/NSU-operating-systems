...
queue_t* queue_init(int max_count) { // аргумент = макс кол-во нод в спсике
	if (pthread_spin_init(&lock, PTHREAD_PROCESS_PRIVATE)) {
		printf("ERROR: spinlock_init in queue_init()\n");
		abort();
	}
...
void queue_destroy(queue_t *q) {
	if (pthread_spin_lock(&lock)) {
		printf("ERROR: spinlock_lock in queue_destroy()\n");
	}
	...
	if (pthread_spin_unlock(&lock)) {
		printf("ERROR: spinlock_unlock in queue_destroy()\n");
	}
	if (pthread_spin_destroy(&lock)) {
		printf("ERROR: spinlock_destroy in queue_destroy()\n");
	}
}
...
int queue_add(queue_t *q, int val) {
	...
	if (err) {
		printf("ERROR: spinlock_lock in queue_add()\n");
	}
	...
	if (q->count == q->max_count) {
		if (pthread_spin_unlock(&lock)) {
			printf("ERROR: spinlock_unlock in queue_add()\n");
		}
		return 0;
	}
...
int queue_get(queue_t *q, int *val) {
	int err = pthread_spin_lock(&lock);
	if (err) {
		printf("ERROR: spinlock_lock in queue_get()\n");
	}
	...
	*val = tmp->val; // достаём значение
	...
	if (pthread_spin_unlock(&lock)) {
		printf("ERROR: spinlock_unlock in queue_get()\n");
	}
